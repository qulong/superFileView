...
/**
 * 根据MonthView 修改而来 为了实现自己的视图规则
 */
public abstract class MySingleMonthView extends BaseMonthView {
...
//                mSchemePaint.setColor(calendar.getSchemeColor() != 0 ? calendar.getSchemeColor() : mDelegate.getSchemeThemeColor());
                onDrawScheme(canvas, calendar, x, y,true,isPreSelected,isNextSelected);
     ...
     * @param y        日历Card y起点坐标
     */
    protected abstract void onDrawScheme(Canvas canvas, Calendar calendar, int x, int y, boolean hasScheme,boolean isSelectedPre, boolean isSelectedNext);
