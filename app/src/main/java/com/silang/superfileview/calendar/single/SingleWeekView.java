package com.silang.superfileview.calendar.single;
...
import com.dy.lib_calendar.MySingleWeekView;
import com.dy.lib_calendar.WeekView;
...
public class SingleWeekView extends MySingleWeekView {
...
//        mSchemePaint.setMaskFilter(new BlurMaskFilter(30, BlurMaskFilter.Blur.SOLID));
    ...
    @Override
    protected void onDrawScheme(Canvas canvas, Calendar calendar, int x, boolean hasScheme,boolean isSelectedPre, boolean isSelectedNext) {
//        int cx = x + mItemWidth / 2;
//        int cy = mItemHeight / 2;
        ...
        if (isSelectedPre) {
            if (isSelectedNext) {
                canvas.drawRect(x, cy - mRadius, x + mItemWidth, cy + mRadius, mSchemePaint);
            } else {//最后一个，the last
